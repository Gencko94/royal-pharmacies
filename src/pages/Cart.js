    ...
    <div className=" cart py-2 px-4 relative" style={{maxWidth:'1560px',margin:'0 auto'}}>
      <div className="font-semibold float-right">
        <div className=" rounded border bg-gray-100 p-2 flex justify-start flex-col mb-2 ">
          ...
          <button className={`${cartItems.length === 0 ? 'cursor-not-allowed bg-gray-600' : 'bg-green-600'} p-1 rounded text-gray-100 `} disabled={cartItems.length === 0}>
            Proceed to Checkout
          </button>
        </div>
        {visitedItems.length > 3
           ? <RecentlyViewedVertical visitedItems={visitedItems} /> : <FeaturedItemsVertical />
        }
      </div>
